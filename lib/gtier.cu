...
struct gtier {
    ...
    // Open tickets for the async path.  Each owns a disjoint slot range, and a
    // completion carries its ticket id alongside the slot so a wait on one
    // ticket can reap and account another's without dropping it.
    struct Inflight {
        bool open = false;
        int outstanding = 0;
        int slot_base = 0;
...
gtier *gtier_open(const char *path, const gtier_config *user) {
    ...
    g->ticket_slots = g->cfg.slots / GTIER_MAX_INFLIGHT;
    for (int i = 0; i < GTIER_MAX_INFLIGHT; ++i)
        g->fly[i].slot_base = i * g->ticket_slots;
...
int gtier_submit(gtier *g, const gtier_range *r, int n, gtier_ticket *t) {
    ...
    auto ps = plan_exact(r, n, g->cfg.merge_gap, g->cfg.slot_bytes, g->file_bytes);
    if (ps.empty()) return -E2BIG;
    if ((int)ps.size() > g->ticket_slots) return -ENOSPC;
    ...
    f.open = true;
    f.outstanding = (int)ps.size();
    f.bytes_read = 0;
    f.useful = 0;
    f.placement.clear();
    for (size_t p = 0; p < ps.size(); ++p)
        for (int m : ps[p].members) {
            f.placement.push_back({(uint64_t)m,
                                   (uint64_t)(f.slot_base + (int)p),
                                   r[m].off - ps[p].off});
            f.useful += r[m].len;
        }
    f.plans = std::move(ps);
...
int gtier_wait(gtier *g, gtier_ticket *t, void **dev_out) {
    if (!g || !t || t->id < 0 || t->id >= GTIER_MAX_INFLIGHT) return -EINVAL;
    gtier::Inflight &me = g->fly[t->id];
    ...
    while (me.outstanding > 0) {
        io_uring_cqe *c;
        if (io_uring_wait_cqe(&g->ring, &c) < 0) return -EIO;
        int id = (int)(io_uring_cqe_get_data64(c) >> 32);
        int res = c->res;
        io_uring_cqe_seen(&g->ring, c);
        if (res < 0) return res;
        // The completion may belong to another open ticket; credit it there and
        // keep reaping until this one is done.
        gtier::Inflight &f = g->fly[id];
        f.bytes_read += res;
        --f.outstanding;
    }
