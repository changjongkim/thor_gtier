...
set -u
# Absolute, because this script is run from a snapshot outside the repo so
# that editing it cannot disturb a run already in flight.
ROOT=/home/thor/kcj/thor_gtier
cd "$ROOT"
LOG=$ROOT/results/SERVE/progress.log
...
"$ROOT/scripts/run_serving_queue.sh"  >> "$ROOT/results/SERVE_queue.out"  2>&1
...
if [ ! -s "$ROOT/results/SCOPE/routing_multi.npz" ]; then
  ...
  . "$ROOT/scripts/torch_env.sh"
  $TORCH_VENV/bin/python scripts/capture_routing.py --decode 64 \
      --out "$ROOT/results/SCOPE/routing_multi.npz" >> "$ROOT/results/capture_multi.out" 2>&1 \
      && $TORCH_VENV/bin/python "$ROOT/scripts/export_routing.py" \
           --npz "$ROOT/results/SCOPE/routing_multi.npz" \
           --out "$ROOT/results/SCOPE/routing_multi.bin" >> "$ROOT/results/capture_multi.out" 2>&1
  say "capture done"
fi

"$ROOT/scripts/run_serving_queue2.sh" >> "$ROOT/results/SERVE_queue2.out" 2>&1
"$ROOT/scripts/run_serving_queue3.sh" >> "$ROOT/results/SERVE_queue3.out" 2>&1
"$ROOT/scripts/run_engine_queue.sh"   >> "$ROOT/results/ENGINE_queue.out"  2>&1

python3 "$ROOT/scripts/summarize_serving.py" > "$ROOT/results/SERVE/SUMMARY.md" 2>/dev/null
cd "$ROOT"
