...
// ring empties between calls and a client issuing back-to-back batches spends
// that gap at zero queue depth while the device idles.  Splitting submit from
// wait lets the next batch go out before the current one is collected, keeping
// the ring full.
//
//   gtier_submit(g, r0, n, &t0);
//   for (;;) {
//       gtier_submit(g, r1, n, &t1);   // next batch out first
//       gtier_wait(g, &t0, dev);       // then collect this one
//       consume(dev);
//       swap(t0, t1);
//   }
//
// Tickets own disjoint slots, so each open ticket takes slots/GTIER_MAX_INFLIGHT
// of the window.
#define GTIER_MAX_INFLIGHT 4

typedef struct { int id; int n; } gtier_ticket;
