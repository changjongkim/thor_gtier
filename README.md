# gTier — 엣지 통합 메모리에서 Out-of-Core MoE LLM 추론 가속
...
> **한 문장.** MoE는 엣지에서 **연산은 맞는데 메모리가 안 맞는** 유일한 모델 구조다.
> Qwen3-235B-A22B는 토큰당 연산이 0.89 ms인데 가중치를 읽는 데 2200 ms가 걸린다 — **2475배**.
> 따라서 엣지 MoE 서빙은 연산 문제가 아니라 **메모리 분할 문제**다: 통합 풀 B를 상주 가중치 R,
> 스테이징 W, KV 캐시 K로 어떻게 쪼갤 것인가. 기존 데이터 경로는 디스크리트 GPU에서 설계되어
> **W가 공짜라고 가정**하고, 선언한 것의 2.18~5.31배를 잡는다. 그 초과분은 R과 K에서 나오고,
> M/B ≈ 1인 지점에서 증폭된다 — **8.3 GiB 초과가 읽어야 할 바이트를 53% 늘린다.**

---

## 1. 서론

### 1.1 왜 MoE인가 — 엣지에서 연산은 맞는데 메모리가 안 맞는다

Dense 모델을 엣지에 올리려면 파라미터 수만큼 연산도 같이 줄여야 한다. MoE는 그 둘을
분리한다 — Qwen3-235B-A22B는 **총 235B인데 토큰당 22B만 활성**이고, Thor 기준으로
그 연산은 이미 넘치도록 빠르다.

| | 값 | 근거 |
|---|---:|---|
| 토큰당 활성 파라미터 | 22B | 8/128 전문가 |
| 토큰당 연산 | 44 GFLOP | 2 FLOP/param |
| **연산 시간** | **0.89 ms** | Thor bf16 49.5 TFLOP/s (실측) |
| 토큰당 읽어야 할 가중치 | 12.5 GiB | Q4_K_M, 8/128 라우팅 (§4.11) |
| **I/O 시간** | **2200 ms** | NVMe 5.682 GiB/s (실측) |
| **I/O / 연산** | **2475x** | |

**기기는 수학을 가중치를 가져오는 것보다 2475배 빨리 한다.** 엣지 MoE 서빙은 전적으로
데이터 이동 문제다. 이것이 이 저장소가 연산이 아니라 I/O 경로를 다루는 이유다.

### 1.2 왜 엣지인가 — 메모리를 더 살 수 없다

서버라면 답이 간단하다. VRAM을 더 사거나 GPU를 늘려 샤딩한다. **엣지에서는 둘 다 불가능하다.**
메모리가 고정이고, 통합되어 있고, **swap이 없다** — 초과는 느려지는 것이 아니라 OOM이다.

그리고 하필 그 고정된 크기가 문제를 흥미롭게 만든다.

```
Qwen3-235B-A22B Q4_K_M  =  132.4 GiB
Thor 통합 메모리        =  122.8 GiB
                           ─────────
                           M/B = 1.08x
```

**"거의" 들어간다.** 전부 스트리밍해야 하는 것도 아니고 전부 상주시킬 수 있는 것도 아니다.
92%를 상주시키고 8%를 스트리밍하는 체제이고, 뒤에서 보듯 **이 체제에서 메모리 1 바이트의
값이 가장 비싸다**(§1.4).

통합 메모리는 엣지만의 것이 아니다 — GH200, MI300A, Apple Silicon이 같은 구조다.
엣지는 그 구조에서 **예산이 가장 빡빡한 지점**이고, 그래서 문제가 가장 선명하게 드러난다.

### 1.3 이 도메인의 문제 — 메모리 분할

M > B이므로 통합 풀 B를 셋으로 쪼개야 한다.

```
B (122.8 GiB)  =  R (상주 가중치)  +  W (스테이징 윈도우)  +  K (KV 캐시 / 배치)

tok/s  =  대역폭 / 토큰당바이트

   토큰당바이트 ∝ (M − R)         ← 상주 못 한 나머지만 읽는다
   토큰당바이트 ∝ 1 / batch(K)    ← 배치가 전문가 합집합을 공유한다
   대역폭       = g(W)            ← 0.5 GiB에서 포화 (§4.14, 측정)
```

세 항의 성질이 다르다. **R과 K는 늘릴수록 좋고, W는 0.5 GiB 위에서 아무것도 사주지 않는다.**
따라서 최적 배분은 자명하다 — **W를 최소로 하고 나머지를 R과 K에 준다.**

문제는 **데이터 경로가 W를 최소화하게 해주지 않는다**는 것이다.

### 1.4 선행연구는 무엇을 풀었고 무엇을 남겼나

| 계열 | 대표 | 풀려던 문제 | Coherent SoC에서 남는 것 |
|---|---|---|---|
| **오프로딩** | ZeRO-Infinity, FlexGen, DeepSpeed-Inf | GPU↔CPU 계층 스케줄링 | 두 계층이 같은 메모리라 스케줄링이 무의미. 호스트 스테이징 + 디바이스 버퍼로 **버퍼 2벌 = W의 2.18배** |
| **GPU 직결 스토리지** | GDS/cuFile, G10, BaM | CPU 바운스 제거 | Thor에 커널 경로 없음(`nvfs 0.0`). **바운스 버퍼로 퇴화하고 그것이 선언 W에 계산되지 않음 — 5.31배** |
| **MoE 전용** | MoE-Infinity, Pre-gated MoE, SiDA-MoE | 라우팅 편향으로 인기 전문가 캐싱 | 캐싱이 이 비율에서 **손해**. 적중률 47.2%인데 증폭 1.24x로 4.321 → 3.547 GiB/s (§3.4) |
| **통합 메모리 / demand paging** | DeepUM, UVM | 드라이버에 맡김 | GPU 폴트가 4 KiB라 입도에 갇힘 — **0.240 GiB/s, 장치의 4.3%** (§4.2) |
| **뉴런 희소성** | PowerInfer, DejaVu | 활성 뉴런 예측 | 접근 단위가 행 단위 ~2 KiB로 **입도 바닥(64 KiB) 아래** (§4.4) |

**공통점.** 전부 디스크리트 GPU에서 설계되었고, 거기서 스테이징 버퍼는 **호스트 DRAM**에
있고 모델은 **VRAM**에 있어 서로 경쟁하지 않는다. 그래서 W를 공짜로 쓴다. 그 전제는
통합 풀에서 깨진다.

### 1.5 그래서 얼마나 손해인가 — M/B ≈ 1에서 증폭된다

선언 윈도우를 똑같이 2.00 GiB로 주고 회수 불가능한 점유를 쟀다(§4.15). K를 4 GiB로 동일하게
두고 132.4 GiB 모델을 122.8 GiB에 올린다고 하면:

| 데이터 경로 | 대응 선행연구 | 실제 W | R = B−W−K | 상주율 | 스트리밍 | **토큰당 바이트** |
|---|---|---:|---:|---:|---:|---:|
| **gTier** | (본 연구) | **2.33** | **116.5** | **88.0%** | **15.9 GiB** | **1.50 GiB** |
| pread+copy | FlexGen, ZeRO-Infinity | 4.35 | 114.5 | 86.4% | 18.0 GiB | 1.69 GiB |
| cuFile | NVIDIA GDS | 10.62 | 108.2 | 81.7% | 24.2 GiB | **2.29 GiB** |

**8.3 GiB의 초과가 모델의 6.3%인데 읽어야 할 바이트를 53% 늘린다.**

이유는 그 8.3 GiB가 **상주하지 못한 나머지 15.9 GiB에서 나오기 때문**이다. M/B가 1에
가까울수록 꼬리가 얇아 마진이 증폭된다. 모델이 메모리의 절반이면 이 레버리지는 없다 —
**엣지에서 M/B ≈ 1인 것이 문제를 만들고 동시에 해법의 값어치를 만든다.**

> 위 표의 R·스트리밍·토큰당 바이트는 측정된 W로부터 **유도한 예측**이다.
> 실제 end-to-end 측정은 미완이다(§5).

### 1.6 기여

1. **문제 규정.** 엣지 MoE 서빙이 연산이 아니라 메모리 분할 문제임을 보이고(I/O가 연산의
   2475배), 통합 풀에서 목적함수가 "최대 대역폭"에서 **"예산 바이트당 스루풋"** 으로
   바뀜을 보인다(§1.1–1.3).

2. **선행 전제의 비용 측정.** 디스크리트 GPU 전제가 통합 풀에서 얼마인지 잰다 —
   선언 W 대비 cuFile **5.31x**, pread+copy **2.18x**, gTier **1.17x**. cuFile은 8 GiB
   예산에서 실행조차 되지 않는다(§4.13, §4.15).
...
5. **음성 결과.** W는 모델의 0.9%에서 포화하고, 슬롯 크기는 256 KiB~16 MiB에서 무관하며,
   전문가 캐싱은 이 비율에서 **손해**다. 스루풋 레버는 배치뿐이다(§4.12, §4.14).

### 1.7 주장하지 않는 것

- **추론 엔진이 아니다.** 가중치를 GPU 주소공간까지 나르는 I/O 계층이고 행렬곱·어텐션·
  KV 캐시를 다루지 않는다. 본 저장소의 벤치마크도 **연산을 하지 않는다** — GPU 커널이
  64바이트마다 한 바이트를 더할 뿐이다. 따라서 `tok/s`는 **가중치 전달률**이지 추론
  처리량이 아니다.
- **§1.5의 레버리지는 아직 예측이다.** 측정된 W로부터 유도했고 end-to-end로 닫지 못했다.
- **대화형 서빙용이 아니다.** 배치 1에서 0.749 tok/s다. 처리량형 워크로드를 겨냥한다.
- **전송 경로 자체의 우위는 넓지 않다.** 유능한 베이스라인(pread+copy) 대비 +16.8%다.
  22.7배는 OS demand paging 대비지 선행 시스템 대비가 아니다.
...
### 2.1 MoE 추론이 스토리지에 내는 요청

`Qwen/Qwen3-30B-A3B`(평가용, 57 GiB)와 `Qwen3-235B-A22B Q4_K_M`(목표, 132.4 GiB)은
구조가 같다.

```
층마다: 전문가 128개 × 텐서 3개(gate/up/down)
        + 어텐션·norm (매 토큰 전부 읽음)
라우터가 토큰마다 층마다 128개 중 8개를 고른다 (6.25%)
```

| | Qwen3-30B-A3B | Qwen3-235B-A22B Q4 |
|---|---:|---:|
| 모델 | 57 GiB | **132.4 GiB** |
| 전문가 비중 | 94.7% | 96.6% |
| 전문가 텐서 크기 | 3 MiB | ~4 MiB |
| 토큰당 가중치 | 6.4 GiB | **12.5 GiB** |
| M / B (122.8 GiB) | 0.46 | **1.08** |

**요청의 모양.** 토큰마다, 층마다, 데이터 의존적으로 고른 **MiB 단위 바이트 범위 수십 개**.
이것이 §3의 설계가 겨냥하는 접근 패턴이다. 두 성질이 중요하다.

- **데이터 의존적이다** — 정적 스케줄로 프리페치할 수 없다. 라우터가 정한다.
- **입도가 MiB다** — §4.4의 입도 바닥(64 KiB)보다 한참 위다. 그래서 **정확히 인출하는 것이
  가능하고 값을 한다.** 벡터 검색(4 KiB 섹터)이나 GNN(수백 B)은 이 조건을 만족하지 못한다.

### 2.2 플랫폼 — 하나의 풀이 만드는 기회와 제약
...
**병목은 링크가 아니라 SSD다.** Gen4 x4가 7.34 GiB/s를 나를 수 있는데 이 DRAM-less
클라이언트 드라이브가 5.68에서 막고, **동시성을 늘리면 오히려 느려진다**(잡 1개 5.682,
...
| 호스트→디바이스 복사 | PCIe 전송 | DRAM→DRAM (127 GB/s) | DRAM→DRAM |
...
GPU가 직접 주소 지정하는 메모리다. 디스크리트 GPU에서는 이 둘이 서로 다른 메모리라 복사나
GDS가 필요하지만 여기서는 같은 버퍼다. §3.2가 이것을 쓴다. **Orin에서는 성립하지 않는다** —
이 메커니즘은 Thor 세대의 coherency를 요구한다.

**(b) 제약.** 같은 통합이 경쟁을 만든다. 데이터 경로가 잡는 모든 바이트가 R과 K에서 나오고,
**swap이 없으므로 초과는 느려지는 것이 아니라 OOM이다.** §3.5가 이것을 다룬다.
...
그리고 **그 바운스 버퍼는 호출자가 선언한 윈도우에 계산되지 않는다** — §1.5의 5.31x가
여기서 나온다.
...
§1.3의 분할 `B = R + W + K`에서 gTier가 책임지는 것은 **W 하나**다. R과 K는 애플리케이션의
것이고, gTier의 일은 **W를 최소로 유지하면서 그 안에서 장치 대역폭을 다 쓰는 것**이다.
...
여기 싣는 이유는 §2.1의 접근 패턴이 실제 모델에서 어떤 크기인지 보이기 위해서다 —
