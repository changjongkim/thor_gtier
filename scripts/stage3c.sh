#!/bin/bash
# Stage 3c: DuoServe-MoE* on the same prompts, budgets and cap as stage 3.
# Popularity and affinity come from the model's other two workloads.
set -u
...
for m in qwen30b mixtral8x7b qwen235b; do
  ...
  for w in longbench sharegpt mmlu; do
    for frac in 0.25 0.45 0.65; do ds $m $w $frac; done
    ds $m $w 0.45 4
  done
  ...
  git diff --cached --quiet || { git commit -q -m "DuoServe* baseline: $m
