...
### 원인 분리 — GPU 폴트에는 커널 readahead가 닿지 않는다
...
> **CPU 스레드 하나가 GPU 전체보다 12.4배 빠르다.** 문제는 mmap도, 4 KiB 페이지 자체도, 스토리지도
> 아니다. 커널 readahead가 CPU 폴트는 큰 I/O로 묶어주지만 **GPU 폴트 경로에는 닿지 않는다.**
> Thor가 새로 준 능력은 **그것을 쓸 만하게 만드는 OS 기구 없이 노출되었다.**
...
|---|---:|---:|
| Qwen2.5-7B Q8 (7.5 GiB) | **2.94x (+194%)** | **+0.3%** |
| Qwen2.5-14B Q8 (14.6 GiB) | **2.70x (+170%)** | **+0.2%** |
| Qwen2.5-32B Q8 (32.4 GiB) | **2.06x (+106%)** | **+0.14%** |
