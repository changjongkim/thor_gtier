# gTier — 하나의 메모리 풀에서 Out-of-Core GPU 데이터 경로
...
> **주장.** 디스크리트 GPU에서 오프로딩 시스템의 스테이징 버퍼는 호스트 DRAM에 있고 모델은
> VRAM에 있어 **서로 경쟁하지 않는다.** 그래서 기존 설계는 스테이징 메모리를 사실상 공짜로
> 쓴다. Coherent SoC에는 풀이 하나뿐이라 그 전제가 깨진다 — **스테이징이 가져간 1 바이트는
> 배치가 못 쓰는 1 바이트다.** 목적함수가 "최대 대역폭"에서 **"예산 바이트당 대역폭"** 으로
> 바뀐다. 이 저장소는 그 비용을 측정하고, 피할 수 있음을 보인다.

**도메인.** 메모리 예산이 제약된 coherent 엣지 SoC 위의 **배치형 out-of-core MoE 서빙**.
대화형 단일 요청이 아니다(§1.4에서 한정한다).

---

## 1. 서론

### 1.1 문제 — 계층이 사라지고 경쟁이 생긴다

메모리보다 큰 모델을 돌리는 시스템 — ZeRO-Infinity, FlashNeuron, DeepUM, FlexGen, G10,
PowerInfer, InfiniGen, NEO — 은 모두 **GPU 메모리 / 호스트 DRAM / 스토리지** 세 계층을
전제하고, 설계 예산의 대부분을 1↔2계층 스케줄링에 쓴다.
...
**시스템 RAM 전체가 VRAM이다.** 1계층과 2계층이 같은 물리 메모리이므로 그 사이의 스케줄링은
의미를 잃는다. 여기까지는 이미 알려진 이야기다.

**덜 알려진 것은 그 다음이다.** 두 계층이 합쳐지면서 **새로운 경쟁이 생긴다.**

| | discrete GPU | **coherent SoC** |
|---|---|---|
| 모델 가중치가 사는 곳 | VRAM (희소) | 하나의 풀 |
| 스테이징 버퍼가 사는 곳 | 호스트 DRAM (풍부, 10배 큼) | **같은 풀** |
| 둘이 경쟁하는가 | **아니오** | **예** |
| 스테이징 1 바이트의 값 | ~0 | **배치 1 바이트** |

디스크리트 GPU에서 FlexGen이 호스트에 큰 스테이징 버퍼를 잡아도 VRAM의 모델은 줄지 않는다.
Coherent SoC에서는 줄어든다. **기존 시스템의 설계는 전부 이 전제를 상속하고 있다.**

### 1.2 그 전제가 실제로 얼마를 먹는가

선언한 윈도우를 똑같이 2.00 GiB로 주고 회수 불가능한 점유를 쟀다(§4.15).

| 데이터 경로 | 대응 선행연구 | 선언 | **실제 점유** | 배수 |
|---|---|---:|---:|---:|
| **gTier** | (본 연구) | 2.00 GiB | **2.33 GiB** | **1.17x** |
| pread+copy | FlexGen, ZeRO-Infinity | 2.00 GiB | 4.35 GiB | 2.18x |
| cuFile | NVIDIA GDS | 2.00 GiB | **10.62 GiB** | **5.31x** |

8 GiB 예산에서 **cuFile은 실행되지 않는다** — cgroup OOM으로 죽고, 같은 일을 하려면
9 GiB가 필요하다. 12 GiB를 다 주고 비교해도 gTier가 4.794 대 3.839로 앞선다(§4.13).

그리고 이 초과분은 그냥 낭비가 아니다. **스루풋이다.** 이 체제에서 tok/s를 올리는 레버는
배치 하나뿐이고(윈도우를 키우는 것은 0.5 GiB 위에서 아무 효과가 없다, §4.14), 배치는
윈도우가 아니라 **KV 캐시**를 먹는다. 배치 1→32가 0.749→3.675 tok/s, **4.9배**다.

### 1.3 기여

1. **문제 규정.** Coherent SoC에서 오프로딩 데이터 경로의 목적함수가 "최대 대역폭"에서
   "예산 바이트당 대역폭"으로 바뀜을 보이고, 기존 설계가 상속한 전제를 짚는다(§1.1, §2.2).

2. **비용 측정.** 그 전제의 값을 잰다 — 선언 윈도우 대비 cuFile 5.31x, pread+copy 2.18x,
   gTier 1.17x. cuFile은 8 GiB 예산에서 실행 불가(§4.13, §4.15).

3. **계측 방법론.** 통합 메모리에서 발자국을 재는 법. 세 계수기가 서로 다른 것을 센다 —
   cgroup은 `cudaMalloc`을 과금하지 않고, RSS도 못 보고, `cudaMemGetInfo`는 매핑된 호스트
   메모리를 중복 계수해 합산이 불가능하다. `MemAvailable` 하락폭만 일관된다. 그리고
   **예산을 고정하지 않으면 결론이 뒤집힌다** — 같은 mmap-cpu가 0.607 ↔ 5.289 GiB/s로
   8.7배 움직인다(§4.15).

4. **설계.** 선언한 예산을 지키면서 대역폭 1위인 데이터 경로. 세 평면으로 나뉜다 —
   스테이징(복사 0회 동일성), 전달(정확 인출 + 연속 제출), 상주(정책)(§3).

5. **음성 결과.** 스테이징 윈도우는 모델의 0.9%에서 포화하고, 슬롯 크기는 256 KiB~16 MiB에서
   무관하며, 캐싱은 이 체제에서 **손해**다(적중률 47.2%인데 증폭 1.24x로 더 느림).
   스루풋 레버는 배치뿐이다(§4.14, §4.12).

### 1.4 주장하지 않는 것

- **추론 엔진이 아니다.** 가중치를 GPU 주소공간까지 나르는 I/O 계층이고, 행렬곱·어텐션·
  KV 캐시를 다루지 않는다. 본 저장소의 벤치마크도 연산을 하지 않는다 — GPU 커널이
  64바이트마다 한 바이트를 더할 뿐이다. 따라서 보고하는 `tok/s`는 **가중치 전달률**이지
  추론 처리량이 아니다.
- **end-to-end 가속을 아직 보이지 못했다.** 탑티어 베이스라인 셋을 모두 빌드했으나
  실제 엔진 대비 end-to-end 숫자는 미확보다(§5).
- **대화형 워크로드용이 아니다.** 배치 1에서 0.749 tok/s다. 배치가 없으면 이 설계의
  스루풋 레버 자체가 없다.
- **전송 경로의 우위는 넓지 않다.** 유능한 베이스라인(pread+copy) 대비 +16.8%다.
  22.7배는 OS demand paging 대비지 선행 시스템 대비가 아니다(§4.2).
...
| `concurrentManagedAccess` | — | **1** (Orin은 0) |
| `pageableMemoryAccess` | — | **1** (Orin은 0) |

여기서 두 가지가 따라 나온다.

**(a) 기회.** `cudaHostAlloc(cudaHostAllocMapped)` 메모리는 **동시에** O_DIRECT DMA 대상이고
GPU가 직접 주소 지정하는 메모리다. 디스크리트 GPU에서는 이 둘이 서로 다른 메모리라 복사
(FlexGen)나 GDS(cuFile)가 필요하지만, 여기서는 같은 버퍼다. §3.2가 이것을 쓴다.

**(b) 제약.** 같은 통합이 경쟁을 만든다. 데이터 경로가 잡는 모든 바이트가 모델·KV 캐시·
동거 테넌트에서 나온다. 그리고 **swap이 없으므로 초과는 느려지는 것이 아니라 OOM이다.**
§3.5가 이것을 다룬다.
...
그리고 **그 바운스 버퍼는 호출자가 선언한 윈도우에 계산되지 않는다** — 측정된 초과분
5.31x의 출처다(§4.15).

다만 **버퍼 등록 자체에는 한계가 없다** — 64 MiB 버퍼 640개, **40 GiB 등록이 정상 동작한다**
([`gtier/cufile_limit.cu`](gtier/cufile_limit.cu)). 앞서 "cuFile이 큰 윈도우를 못 잡는다"고
적었던 것은 철회한다(§5).

### 2.4 워크로드 — MoE 라우팅은 희소 바이트 범위 문제다

`Qwen/Qwen3-30B-A3B`, safetensors 16 샤드, 57 GiB, bf16.

```
48개 층
  각 층: 전문가 128개 × 텐서 3개(gate/up/down) × 3 MiB = 1.125 GiB
         + 어텐션·norm
  전문가 전체 = 54 GiB,  나머지 = 3 GiB
```

토큰마다 라우터가 **층마다 128개 중 8개**(6.25%)를 고른다.

| | 계산 | 바이트 |
|---|---|---:|
| 전문가 | 48 × 8 × 3 × 3 MiB | 3.375 GiB |
| 어텐션·임베딩 (매 토큰 전부) | — | 3.0 GiB |
| **토큰당 합계** | | **6.4 GiB** |

모델이 57 GiB라 예산 안에 상주시킬 수 없으므로 **토큰마다 6.4 GiB를 스토리지에서 새로
읽어야 한다.** 읽어야 할 것은 데이터 의존적으로 흩어진 `(파일, 오프셋, 길이)` 목록이고,
항목 입도는 3 MiB로 §4.4의 입도 바닥(64 KiB)보다 한참 위다. 이것이 gTier가 겨냥하는
접근 패턴의 정의다.
...
gTier는 추론 엔진이 아니라 **바이트 범위 전달 계층**이다. 애플리케이션이 원하는 범위를
선언하면 GPU가 읽을 수 있는 포인터를 돌려준다.

```
   애플리케이션          gtier_submit(ranges) ─────────────┐
   (라우터가 고른 범위)                                      │
                                                            ▼
 ┌────────────────────────────────────────────────────────────────────┐
 │  § 3.3  전달 평면   planner → io_uring(O_DIRECT) → 연속 제출        │
 │         "정확히 원한 바이트만, 링을 비우지 않고"                     │
 ├────────────────────────────────────────────────────────────────────┤
 │  § 3.4  상주 평면   NONE / BLOCK / ADAPTIVE / PIN                   │
 │         "다시 읽을 것만 남긴다 — 이 체제에서는 대개 손해"            │
 ├────────────────────────────────────────────────────────────────────┤
 │  § 3.2  스테이징 평면   cudaHostAlloc(Mapped) 슬롯 링               │
 │         "착륙지가 곧 GPU 포인터 — 복사 0회"                          │
 └────────────────────────────────────────────────────────────────────┘
                                                            │
   § 3.5  예산 계약: 세 평면이 쓰는 총량 = 선언한 윈도우      ▼
                                                    GPU 커널이 직접 역참조
```

각 평면은 같은 형태로 읽는다 — **선행연구가 하는 것 → coherent SoC에서 깨지는 것 →
우리 설계 → 측정된 이득.**

### 3.2 스테이징 평면 — 착륙지가 곧 GPU 포인터

**선행연구.** 스토리지에서 GPU로 가는 길은 둘 중 하나다. 호스트 버퍼에 읽고 복사하거나
(FlexGen, ZeRO-Infinity), GDS로 직접 DMA하거나(cuFile). 전자는 복사 1회, 후자는 전용
커널 드라이버가 필요하다.

**깨지는 것.** Thor에는 GDS 커널이 없어(§2.3) cuFile이 바운스 버퍼로 퇴화한다 — 복사가
사라지기는커녕 **숨은 메모리가 붙는다.** 한편 복사 경로는 복사 자체보다 **버퍼를 두 벌
잡는 것**이 문제다. 하나의 풀에서 그것은 예산 2배다.

**설계.** `cudaHostAlloc(cudaHostAllocMapped)`으로 잡은 슬롯 하나가 동시에 O_DIRECT DMA
대상이고 GPU 주소다. **드라이브가 GPU가 읽을 메모리에 직접 쓴다.**
...
**이득.** 선언 윈도우 2.00 GiB 대비 실제 점유 **2.33 GiB (1.17x)**. 같은 조건에서
pread+copy 4.35 (2.18x), cuFile 10.62 (5.31x)다(§4.15). 복사가 사라지는 것은
**대역폭이 아니라 전송당 지연(~20 µs)** 이고 — coherent SoC의 복사 대역폭 127 GB/s는
스토리지보다 23배 빠르다 — **실질 이득은 버퍼를 한 벌만 잡는 것이다.**

### 3.3 전달 평면 — 정확히, 그리고 링을 비우지 않고

**선행연구.** 블록 단위로 읽고 필요 없는 부분은 버린다(페이지 캐시, mmap, 블록 캐시).
또는 인접 요청을 병합해 큰 I/O를 만든다.
...
- **규칙 1: 고립된 범위를 증폭하지 마라.** 곡선이 선형보다 느리게 자라므로 증폭이
  언제나 이득을 앞지른다.
- **규칙 2: 인접 범위도 병합하지 마라.** 깊은 큐에서 요청은 거의 공짜이고 바이트만 값을 한다.

**보정은 모델이 아니라 측정으로.** 파라메트릭 모델(고정 요청비용 + 바이트당 비용)로
임계값을 유도했다가 틀렸다. 현재는 planner를 후보 임계값들로 실제 돌려 최선을 고른다.

**설계 — 연속 제출.** `gtier_fetch`는 제출 후 전부 기다려 **인출 사이에 큐가 0으로
비워졌다.** 티켓 기반 `submit`/`wait`이 다음 배치를 먼저 내보내고 이번 것을 거두어 링을
채운 상태로 유지한다. 티켓은 겹치지 않는 슬롯을 소유하고, CQE의 `user_data`에 티켓 id를
실어 다른 티켓의 완료를 만나도 잃지 않는다.
...
(배치 64). cuFile 대비 1.29x ~ 1.07x다(§4.6). 반면 **이중 버퍼링은 이득이 없다**
(0.98~1.01x) — 병목이 드레인이었기 때문이다(§4.12).

### 3.4 상주 평면 — 그리고 이 체제에서는 대개 손해다

**선행연구.** MoE 전문가 캐싱(MoE-Infinity, Pre-gated MoE, SiDA-MoE)은 라우팅의 편향을
이용해 인기 전문가를 상주시킨다.

**깨지는 것.** 캐시는 고정 블록을 요구하고 그것은 규칙 1이 금지하는 증폭이다. 재사용이
있으면 상환되므로 교차점이 존재하고, 그 교차점은 **240배로 날카롭다**(§4.8).
...
**측정된 결론은 음성이다.** 토큰마다 재라우팅하는 실제 MoE 트레이스에서, 윈도우가 모델의
3.5%일 때:
...
**적중률 47.2%인데 더 느리다.** 4 MiB 블록에 3 MiB 전문가 텐서가 정렬되지 않아 증폭
1.24x가 붙고, 24% 더 읽어서 47% 맞히면 남는 게 없다. 상주가 이기는 것은 **모델이 통째로
들어갈 때뿐**이다 — 윈도우 104.1%에서 0.4405 tok/s 대 3.5%에서 0.1430 (3.1배, §4.9).

### 3.5 예산 계약 — 무엇을 약속하고 어떻게 지키는가

**선행연구가 약속하지 않는 것.** 오프로딩 시스템은 버퍼 크기를 설정으로 받지만, 그것이
프로세스 총 점유의 상한이라고 약속하지 않는다. 디스크리트 GPU에서는 그래도 됐다 —
초과분이 풍부한 호스트 DRAM에서 나왔기 때문이다.

**깨지는 것.** 하나의 풀에서는 초과분이 배치에서 나온다. 그리고 swap이 없으므로 **초과는
느려지는 것이 아니라 OOM이다.** 실제로 cuFile은 8 GiB 예산에서 실행되지 않는다.

**설계.** gTier가 잡는 것은 슬롯 링 하나뿐이다 — `slots × slot_bytes`. 상주 정책은 그
링 안에서 슬롯을 나눠 쓰고(별도 캐시 메모리를 잡지 않고), 비동기 티켓도 그 링을
`GTIER_MAX_INFLIGHT`등분해 쓴다. **세 평면이 같은 예산을 공유한다.**
...
**그리고 지키는 것이 성능이다.** 윈도우를 키우는 것은 0.5 GiB(모델의 0.9%) 위에서
아무 효과가 없다 — 윈도우는 캐시가 아니라 in-flight 버퍼이기 때문이다(§4.14). 스루풋을
올리는 유일한 레버는 배치이고, 배치는 윈도우가 아니라 KV 캐시를 먹는다. **따라서 예산을
지켜 남긴 메모리가 그대로 스루풋이다** — 배치 1→32가 4.9배다.
...
| 절 | 묻는 것 | 대응 기여 |
|---|---|---|
| 4.1 | 3계층 전제가 이 플랫폼에서 성립하는가 | §1.1 문제 규정 |
...
여기 싣는 이유는 §2.4의 접근 패턴이 실제 모델에서 어떤 크기인지 보이기 위해서다 —
그리고 그 희소성이 §3.3의 "정확히 인출하라"를 값어치 있게 만드는 조건이기 때문이다.
