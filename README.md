...
모든 숫자는 **유효 대역폭(GiB/s)** 이다 — 호출자가 *실제로 요청한* 바이트를 경과 시간으로 나눈 값.
증폭이 있으면 장치가 읽은 바이트와 쓸모 있는 바이트가 달라지므로 후자로 잰다. 아래 조건은 모두
증폭 1.00배(요청한 만큼만 읽음)다.

| 용어 | 의미 |
|---|---|
| **item** | 요청 한 건의 크기(바이트). 희소 접근의 입도 |
| **n** | 한 번의 인출에서 요청하는 범위 개수 |

**조건:** 32 GiB 실데이터 파일(비희소·비압축), 16 GiB 구간 무작위 오프셋, 4096 정렬,
실행마다 페이지 캐시 비움, 백엔드마다 별도 프로세스.
**장치 상한:** 순차 5.59 GiB/s, 1 MiB 랜덤 4.98 GiB/s.

#### 백엔드

| 백엔드 | 하는 일 | 대응하는 선행 설계 |
|---|---|---|
| **gtier** | NVMe가 GPU 주소지정 메모리에 직접 DMA, 복사 0회 | 본 연구 |
| cufile | NVIDIA GDS API. 이 플랫폼은 GDS 부재라 바운스+복사 | NVIDIA 공식 |
| pread+copy | `pread` → `cudaMemcpy` → 디바이스 버퍼 | FlexGen / ZeRO-Infinity |
| mmap-cpu | mmap 후 CPU 스레드가 폴트 | 커널 readahead 경로 |
| uvm | `cudaMallocManaged` + `cudaMemPrefetchAsync` | DeepUM |
| mmap-gpu | mmap 후 **GPU가 폴트** | Thor 하드웨어가 제공하는 경로 |

#### 합성 워크로드 — 유효 대역폭 (GiB/s), 높을수록 좋음

| item | n | 인출당 총량 | **gtier** | cufile | pread+copy | mmap-cpu | uvm | mmap-gpu |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 16 KiB | 256 | 4 MiB | **1.817** | 0.650 | 0.658 | 0.114 | 0.125 | 0.040 |
| 64 KiB | 64 | 4 MiB | **3.281** | 1.720 | 1.609 | 0.310 | 0.356 | 0.120 |
| 256 KiB | 16 | 4 MiB | **4.059** | 2.782 | 2.645 | 0.847 | 0.812 | 0.299 |
| 1 MiB | 8 | 8 MiB | **5.084** | 3.960 | 3.695 | 1.368 | 1.327 | 0.727 |
| 4 MiB | 8 | 32 MiB | **5.154** | 4.459 | 4.251 | 3.218 | 1.729 | 1.536 |

(1 MiB·4 MiB 행은 `n`의 하한 8에 걸려 인출당 총량이 4 MiB를 넘는다.)

**배율:**

| item | gTier ÷ cuFile | gTier ÷ mmap-gpu |
|---:|---:|---:|
| 16 KiB | **2.80배** | 45.4배 |
| 64 KiB | **1.91배** | 27.3배 |
| 256 KiB | **1.46배** | 13.6배 |
...
**읽는 조각이 작을수록 gTier가 유리하다.** 복사의 전송당 고정비(~20 µs)가 짧은 읽기에서 비중이
커지기 때문이다([`results/COPY_COST.md`](results/COPY_COST.md)). 연속 제출(`gtier_submit`/`gtier_wait`)
적용 후 수치이며, 동기식 인출은 배치 사이에 큐를 비워 얕은 깊이에서 최대 31.7%를 잃었다
