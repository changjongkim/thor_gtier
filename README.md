...
**남은 일.** 현재 `PIN`은 채우는 순서를 호출자가 준다. **층별 인기 순위를 온라인으로
추정해 자동으로 채우는 것**은 미구현이다. §1.8의 한계에 적었듯 지금 수치는 디코드 통계로
사후에 순서를 정한 값이다.
...
**열린 질문.** 4번이 진짜 설계 문제다. 프리픽스 전용 상주는 그 프리픽스가 다시 올 때만
값을 하고, 전역 인기 상주는 항상 값을 한다. 배분은 프리픽스 재사용률의 함수이며,
그 분포는 아직 측정하지 못했다.
...
**열린 질문.** 전환 비용이다. R을 다시 채우는 것은 I/O이므로, 전환이 잦으면 이득을 먹는다.
연속 요청이 프리필/디코드를 번갈아 밟는 서빙에서 이것이 성립하는지는 미측정이다.
